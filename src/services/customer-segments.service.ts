...
export async function getSegmentMembers(
  ...
  // Backend's success() helper spreads pagination meta at the top level of
  // the response (flat page/limit/total/totalPages), not nested under a
  // `.pagination` key like Bakaloo's — reconstruct the nested shape here so
  // the rest of the ported UI code doesn't need to change.
  const { data } = await api.get<
    ApiResponse<SegmentMember[]> & { page?: number; limit?: number; total?: number; totalPages?: number }
